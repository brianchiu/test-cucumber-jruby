...
Then(/^I should see the following Web Content is "([^"]*)"$/) do |status, table|
	...
	table.raw.each do |content, title|
		expect(WebContentDetail._workflow_status).to have_text status
...
When(/^I assign to myself an? "([^"]*)" task for the following Web Content$/) do |task, table|
	...
	table.raw.each do |content, title|
		...
		MyWorkflowTasksTask.vertical_ellipsis.click
...
When(/^I should see the following Web Content displayed in the portlet$/) do |table|
	table.raw.each do |content, title|
		expect(WebContentDisplayPortlet._webcontent_content).to have_text content
		expect(WebContentDisplayPortlet._webcontent_title).to have_text title
	end
end